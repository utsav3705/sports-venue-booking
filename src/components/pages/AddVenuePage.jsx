 ...
 * - onSuccess (function): Called with the new venue after creation.
 */
export default function AddVenuePage({ onBack, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || "Failed to create venue. Please try again.";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  if (success && createdVenue) {
    return (
      <main className="max-w-lg mx-auto px-4 sm:px-6 py-16 text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground mb-2">Venue Added!</h2>
        <p className="text-muted-foreground mb-6">
          <span className="font-semibold text-foreground">{createdVenue.name}</span> is now live and visible to all users.
        </p>
        <div className="bg-card border border-border rounded-2xl p-4 text-left mb-6 space-y-2">
          {[
            ["Name", createdVenue.name],
            ["Sport", createdVenue.sport],
            ["Area", createdVenue.area],
            ["Price", `₹${createdVenue.pricePerHour}/hr`],
            ["Hours", `${createdVenue.openTime} – ${createdVenue.closeTime}`],
          ].map(([k, v]) => (
        ...
        <div className="flex gap-3">
          <button
            onClick={() => onSuccess(createdVenue._id)}
          ...
          <button
            onClick={onBack}
            className="flex-1 py-3 rounded-xl border border-border text-foreground font-semibold text-sm hover:bg-secondary transition-colors"
          >
            Back to Venues
          </button>
        </div>
    ...
    <main className="max-w-xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">List your sports venue for bookings</p>
        </div>
      </div>
